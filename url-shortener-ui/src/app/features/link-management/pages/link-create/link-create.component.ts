...
import {
  ...
  selectLinksCreating,
} from '../../../../state/links/link.selectors';
...
import { urlValidator, customAliasValidator } from '../../../../shared/validators/url.validator';
import { AliasAsyncValidator } from '../../../../shared/validators/alias-async.validator';
          ...
          <mat-card-content>
            <form [formGroup]="createForm" (ngSubmit)="onSubmit()" novalidate>
              ...
              <mat-form-field appearance="outline" class="full-width">
                ...
                <mat-error *ngIf="getError('customAlias', 'aliasTaken')">
...
export class LinkCreateComponent implements OnInit {
  ...
  private readonly aliasValidator = inject(AliasAsyncValidator);
  readonly creating$: Observable<boolean> = this.store.select(selectLinksCreating);

  ngOnInit(): void {
    this.createForm = this.fb.group({
      url: ['', [Validators.required, urlValidator()]],
      customAlias: ['', { validators: [customAliasValidator()], asyncValidators: [this.aliasValidator] }],
      expiresAt: [null],
    });
